        ...
        <scroll-view :scroll-top="0"
            ...
            <view class="search-box">
                <view>
                    <icon class="uni-icon"
                          type="search"
                          size="18" /><text>搜索</text>
                </view>
            </view>
            <view class="fn-item"
                  v-for="(item, index) in navList"
                  :key="index"
                  @click="hangelFn(item.code)">
                <view class="icon-box"
                      :style="{ background: item.bgColor }">
                    <uni-icons :type="item.type"
                               :color="item.color"
                               size="26"></uni-icons>
                    <min-badge v-if="item.friendsReq"
                               class="badge"
                               :count="$store.state.unreadNum"></min-badge>
                </view>
                <text>{{ item.text }}</text>
            </view>
            <view class="fn-item"
                  v-for="(item, index) in $store.state.myFriendList"
                  :key="item+index"
